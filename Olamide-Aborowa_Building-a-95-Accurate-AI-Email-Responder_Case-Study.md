...
A preliminary 20-case engineering benchmark reached 95% exact-case accuracy, 100% route accuracy, and zero false auto-ready outcomes. A later 66-email stress run produced 23 approved information requests, 9 assisted-review packages, and 34 human-review-needed outcomes.

Those results are encouraging, but they are not yet a production claim. Operations-approved labels, larger samples, repeated runs, and adversarial cases are still required.
...
1. Validate evaluation labels with parking-operations owners.
2. Expand the benchmark with realistic, ambiguous, and adversarial messages.
3. Measure repeatability, unsafe automation, and eligible automation coverage.
4. Improve local inference latency and throughput.
5. Continue simplifying the operator experience.
6. Add new industry workflows only after the current slice is demonstrably trustworthy.
